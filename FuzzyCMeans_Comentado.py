...
import skfuzzy as fuzz # Biblioteca skfuzzy contendo o algoritmo Fuzzy C-Means
...
y_test = wine_shuffle['target'][130:] # Fazendo o mesmo que no código acima, porém para a coluna "target", das respostas
x_test.head() # Conferindo o código acima

#Implementa o Algoritmo Fuzzy C-means
cntr, u_train, u0_train, d_train, jm_train, p_train, fpc_train = fuzz.cluster.cmeans(
    x_train.transpose(), 3, 3, error=0.005, maxiter=1000, init=None) # Treinando o algoritmo

# Testando o algoritmo
u_test, u0_test, d_test, jm_test, p_test, fpc_test = fuzz.cluster.cmeans_predict(
    x_test.transpose(), cntr, 2, error=0.005, maxiter=1000) # Prevendo os novos valores
prediction = np.argmax(u_test, axis=0) # Salvando a matriz dos resultados obtidos na variável "prediction"
...
plt.title('Resultados Obtidos vs Resultados Reais (Fuzzy C-Means)') # Título do gráfico
